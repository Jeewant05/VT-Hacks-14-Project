...
class OrchestrationKernel:
    ...
    async def plan(self, agent_id: str, intention: IntentionDocument | None = None) -> OrchestrationState:
        state = read_orchestration_state(self.db_path)
        if state.objective is None:
            raise OrchestrationError("create an objective before planning")
        self._agent(state, agent_id)
        ...
        verification = await self.identity.verify(AgentPrincipal(id=agent_id, ans_name=agent_id, role="demo"))
    ...
    async def execute(self, agent_id: str) -> OrchestrationState:
        state = read_orchestration_state(self.db_path)
        workstream = next((w for w in state.workstreams if w.agent_id == agent_id), None)
